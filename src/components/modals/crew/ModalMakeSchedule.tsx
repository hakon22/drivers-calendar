...
const ModalMakeSchedule = () => {
  ...
  const [sortableUsers, setSortableUsers] = useState(shiftOrder.length ? [...shiftOrder.map((orderId) => users.find((usr) => usr.id === orderId) as UserModel), ...users.map((usr) => !shiftOrder.includes(usr.id))] : users);
